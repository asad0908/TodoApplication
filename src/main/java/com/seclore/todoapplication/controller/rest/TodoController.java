...
import com.seclore.todoapplication.service.Todo.TodoServiceInterface;
...
public class TodoController {
	...
	ResponseEntity<?> createTodo(@RequestBody Todo todo, @RequestAttribute("userId") int userId) {
		boolean success = todoService.createTodo(todo, userId);
		if(success)
			return new ResponseEntity<ResponseDTO>(new ResponseDTO("Created a new todo!"), HttpStatus.CREATED);
		return new ResponseEntity<ErrorDTO>(new ErrorDTO(HttpStatus.NOT_MODIFIED, "Failed to create todo!"), HttpStatus.NOT_MODIFIED);
	}
	...
	ResponseEntity<?> updateTodoById(@RequestBody Todo todo, @PathVariable int id) {
		todo.setId(id);
		boolean success = todoService.editTodo(todo);
		if(success)
			return new ResponseEntity<ResponseDTO>(new ResponseDTO("Edited todo!"), HttpStatus.OK);
		return new ResponseEntity<ErrorDTO>(new ErrorDTO(HttpStatus.NOT_MODIFIED, "Failed to edit todo!"), HttpStatus.NOT_MODIFIED);
	}
	...
	ResponseEntity<?> deleteTodoById(@PathVariable(name = "id") int todoId) {
		boolean success = todoService.deleteTodo(todoId);
		if(success)
			return new ResponseEntity<ResponseDTO>(new ResponseDTO("Deleted todo!"), HttpStatus.OK);
		return new ResponseEntity<ErrorDTO>(new ErrorDTO(HttpStatus.NOT_MODIFIED, "Failed to delete todo!"), HttpStatus.NOT_MODIFIED);
	}
